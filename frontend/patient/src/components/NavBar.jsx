...
const NavBar = () => {
    ...
    const { getUser, logout, user, isAuthenticated } = useAuth();
    ...
    return (
        <div className='flex items-center justify-between text-sm py-4 mb-2 border-b border-gray-100'>
            {/* Logo */}
            <div>
                <Logo />
            </div>
            ...
            {/* Right Side: Profile Dropdown or Login Button */}
            <div className='flex items-center gap-4'>
                {isAuthenticated && user ? (
                    <div
                    ref={dropdownRef}
                    className='flex items-center gap-2 cursor-pointer relative outline-none'
                    onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                    >
                        {/* Profile Image and Dropdown Icon */}
                        <img className='w-8 rounded-full' src={user?.user?.image || assets.profile_placeholder} alt="Profile" />
                ...
                <div className={`${showMenu ? 'fixed w-full' : 'h-0 w-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all`}>
                    <div className='flex items-center justify-between px-5 py-6'>
                        {/* <img className='w-36 h-36' src={assets.logo} alt="Logo" /> */}
                        <Logo />
